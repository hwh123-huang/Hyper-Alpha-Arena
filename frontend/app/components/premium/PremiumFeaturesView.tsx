...
export default function PremiumFeaturesView({ onAccountUpdated, onPageChange }: PremiumFeaturesViewProps) {
  ...
  const isPremium = true // All users are premium - no subscription limits
  const maxAllowedDepth = 60 // All users can access full depth
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      {[10, 20, 30, 40, 50, 60].map((depth) => (
                        ...
                        >
                          {depth}
                          {/* Lock icon removed - all features unlocked */}
                        </Button>
                      ))}
                    </div>
                  </div>
