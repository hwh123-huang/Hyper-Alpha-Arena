...
def place_ai_driven_crypto_order(
    ...
    try:
        ...
        if account_id is not None:
            ...
            if (
                not account
                or account.is_active != "true"
                or account.auto_trading_enabled != "true"
            ):
                logger.debug(
                    f"Account {account_id} not found, inactive, or auto trading disabled, skipping AI trading"
                )
                return
            accounts = [account]
        else:
            ...
            if account_ids is not None:
                ...
                if not accounts:
                    logger.debug(
                        "No matching accounts for provided IDs: %s", account_ids
                    )
                    return
        ...
        if available_symbols:
            logger.info(
                f"Available sampling pool symbols: {', '.join(available_symbols)}"
            )
        else:
        ...
        for account in accounts:
            try:
                ...
                logger.info(
                    f"Processing Hyperliquid trading for account {account.name}"
                )
...
def place_random_crypto_order(max_ratio: float = 0.2) -> None:
    ...
    try:
        ...
        if total_assets <= 0:
            logger.debug(
                "Account %s total assets non-positive, skipping auto order placement",
                account.name,
            )
            return
        ...
        if not side_info:
            logger.debug(
                "Account %s has no executable direction for %s, skipping",
                account.name,
                symbol,
            )
            return
...
def place_ai_driven_hyperliquid_order(
    ...
    try:
        ...
        logger.error(
            f"Error in place_ai_driven_hyperliquid_order start: {e}", exc_info=True
        )
        return
    ...
    try:
        ...
        if account_id is not None:
            ...
            if (
                not bypass_auto_trading
                and getattr(account, "auto_trading_enabled", "false") != "true"
            ):
                logger.debug(
            ...
            if account_ids is not None:
                ...
                if not accounts:
                    logger.debug(
                        f"No matching Hyperliquid accounts for provided IDs: {account_ids}"
                    )
                    return
    finally:
        db.close()
    ...
    if available_symbols:
        logger.info(
            f"Available sampling symbols for Hyperliquid: {', '.join(available_symbols)}"
        )
    else:
    ...
    for account in accounts:
        ...
        try:
            ...
            logger.info(
                f"Processing Hyperliquid trading for account: {account.name} (environment: {environment})"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to get Hyperliquid client for {account.name}: {client_err}"
                )
                continue
            ...
            try:
                ...
                decision_kwargs["prompt_template_id"] = (
                    binding.prompt_template_id if binding else None
                )
            except Exception as e:
                logger.warning(
                    f"Failed to get prompt_template_id for {account.name}: {e}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to get account state for {account.name}: {state_err}"
                )
                continue
            ...
            try:
                ...
                logger.info(
                    f"Account {account.name} has {len(positions)} open positions"
                )
            ...
            if not decisions:
                logger.warning(
                    f"Failed to get AI decision for {account.name}, skipping"
                )
                continue
            ...
            ordered_decisions = sorted(
                decisions,
                key=lambda d: decision_priority.get(
                    str(d.get("operation", "")).lower(), 4
                ),
            )
            ...
            for decision in ordered_decisions:
                if not isinstance(decision, dict):
                    logger.warning(
                        f"Skipping malformed Hyperliquid decision for {account.name}: {decision}"
                    )
                    continue
                ...
                leverage = int(
                    decision.get("leverage", getattr(account, "default_leverage", 1))
                )
                ...
                if operation not in ["buy", "sell", "hold", "close"]:
                    logger.warning(
                        f"Invalid operation '{operation}' from AI for {account.name}"
                    )
                    save_ai_decision(
                        db,
                        account,
                ...
                if operation == "hold":
                    ...
                    logger.info(
                        f"AI decided to HOLD for {account.name} - no action taken"
                    )
                    save_ai_decision(
                        db,
                        account,
                ...
                if symbol not in symbol_whitelist:
                    logger.warning(
                        f"Symbol '{symbol}' not in Hyperliquid watchlist for {account.name}"
                    )
                    save_ai_decision(
                        db,
                        account,
                ...
                if target_portion <= 0 or target_portion > 1:
                    logger.warning(
                        f"Invalid target_portion {target_portion} from AI for {account.name}"
                    )
                    save_ai_decision(
                        db,
                        account,
                ...
                if operation == "buy":
                    ...
                    time_in_force = decision.get(
                        "time_in_force", "Ioc"
                    )  # Default to Ioc (market-like)
                    tp_execution = decision.get(
                        "tp_execution", "limit"
                    )  # Default to limit (attempts maker)
                    sl_execution = decision.get(
                        "sl_execution", "limit"
                    )  # Default to limit
                    ...
                    if max_price is not None:
                        ...
                        price_to_use, price_deviation_percent, _ = (
                    ...
                    if order_result and order_result.get("status") == "error":
                        ...
                        if (
                            ...
                            if order_result and order_result.get("status") in [
                                "filled",
                                "resting",
                            ]:
                                logger.info(
                                    f"✅ GTC fallback order succeeded for BUY {symbol}"
                                )
                    ...
                    if order_result and order_result.get("status") == "error":
                        ...
                        if (
                            ...
                            if order_result and order_result.get("status") in [
                                ...
                                logger.info(
                                    f"✅ GTC fallback order succeeded for SELL {symbol}"
                                )
                        ...
                        if not fallback_position:
                            ...
                            if should_cancel_orders:
                                try:
                                            ...
                                            if order_id:
                                                try:
                                                    if client.cancel_order(
                                                        ...
                                                        cancelled_count += 1
                                                        logger.info(
                                                            f"[CLOSE {symbol}] Cancelled {order_type} order #{order_id}"
                                                        )
                                                except Exception as cancel_err:
                                                    logger.warning(
                                                        f"[CLOSE {symbol}] Failed to cancel order #{order_id}: {cancel_err}"
                                                    )

                                        if cancelled_count > 0:
                                            logger.info(
                                                f"[CLOSE {symbol}] Successfully cancelled {cancelled_count}/{len(pending_orders)} pending orders"
                                            )
                                            save_ai_decision(
                                                db,
                                                account,
                                    ...
                                    logger.warning(
                                        f"[CLOSE {symbol}] Failed to fetch pending orders: {orders_err}"
                                    )

                            logger.warning(
                                f"Unable to locate Hyperliquid position data for {symbol}; skipping close."
                            )
                            save_ai_decision(
                                db,
                                account,
                    ...
                    if position_size <= 0:
                        logger.warning(
                            f"No position to close for {symbol} (size={position_size}), skipping close operation"
                        )
                        save_ai_decision(
                            db,
                            account,
                    ...
                    if is_long:
                        ...
                        ai_close_price = (
                            max_price_close
                            if max_price_close is not None
                            else min_price
                        )
                        price_field_used = (
                            "max_price" if max_price_close is not None else "min_price"
                        )
                    ...
                    if ai_close_price:
                        ...
                        if not is_long and close_price < current_price:
                            ...
                            close_price, price_deviation_percent, _ = (
                                _enforce_price_bounds(
                                    symbol=symbol,
                                    account_name=account.name,
                                    operation="close",
                    ...
                    if is_long:
                        price_multipliers = [
                            ...
                            0.992,
                            0.99,
                        ]  # Selling: 0.6% coverage, 4 sampling points
                    else:
                        price_multipliers = [
                            1.004,
                            1.006,
                            1.008,
                            1.01,
                        ]  # Buying: 0.6% coverage, 4 sampling points
                    ...
                    while retry_count < max_retries and order_result is None:
                        ...
                        if retry_count == 0:
                            ...
                            attempt_price = (
                                current_price_retry * price_multipliers[retry_count]
                            )
                        ...
                        error_msg = (
                            attempt_result.get("error", "") if attempt_result else ""
                        )
                        should_retry = (
                            "could not immediately match" in error_msg.lower()
                            or "no resting orders" in error_msg.lower()
                    ...
                    if (
                        not order_result or order_result.get("status") != "filled"
                    ) and not fallback_gtc_attempted:
                ...
                if order_result:
                    ...
                    if order_status in ("filled", "resting"):
                        decision_kwargs["hyperliquid_order_id"] = order_result.get(
                            "order_id"
                        )
                                        ...
                                        if oid:
                                            try:
                                                if client.cancel_order(db, oid, symbol):
                                                    logger.info(
                                                        f"[CLOSE {symbol}] Cancelled {otype} order #{oid}"
                                                    )
                                            except Exception as cancel_err:
                                                logger.warning(
                                                    f"[CLOSE {symbol}] Failed to cancel order #{oid}: {cancel_err}"
                                                )
                            except Exception as orders_err:
                                logger.warning(
                                    f"[CLOSE {symbol}] Failed to fetch remaining orders: {orders_err}"
                                )

                        try:
                            from database.snapshot_connection import (
                                SnapshotSessionLocal,
                            )
                            ...
                            try:
                                trade_record = HyperliquidTrade(
                                    ...
                                    side=operation,
                                    quantity=Decimal(
                                        str(order_result.get("filled_amount", 0))
                                    ),
                                    price=Decimal(
                                        str(order_result.get("average_price", 0))
                                    ),
                                    leverage=leverage,
                                    order_id=order_id,
                                ...
                                snapshot_db.commit()
                                logger.info(
                                    f"[HYPERLIQUID] Trade record saved for {account.name}"
                                )
                            finally:
                                snapshot_db.close()
                        except Exception as trade_err:
                            logger.warning(
                                f"Failed to save Hyperliquid trade record: {trade_err}"
                            )
            ...
            logger.error(
                f"Error processing Hyperliquid account {account.name}: {account_err}",
                exc_info=True,
            )
            db.rollback()
        finally:
...
def place_ai_driven_binance_order(
    ...
    try:
        if account_id is not None:
            ...
            if (
                ...
                logger.debug(
                    f"Account {account_id} auto trading disabled - skipping Binance AI order"
                )
                return
    ...
    if not selected_symbols:
        logger.warning(
            "[Binance] No Binance watchlist configured, skipping Binance trading"
        )
        return
    ...
    for account in accounts:
        ...
        try:
            ...
            if not environment:
                logger.info(
                    f"AI Trader '{account.name}' skipped - No trading environment configured"
                )
                continue
            ...
            if (
                not wallet
                or not wallet.api_key_encrypted
                or not wallet.secret_key_encrypted
            ):
                logger.info(
                    f"AI Trader '{account.name}' (ID: {account.id}) skipped - "
            ...
            try:
                ...
                logger.error(
                    f"Failed to get Binance account state for {account.name}: {e}"
                )
                continue
            ...
            logger.error(
                f"Error processing Binance account {account.name}: {e}", exc_info=True
            )
            db.rollback()
        finally:
            db.close()
...
def _execute_binance_decision(
    ...
    if operation not in ["buy", "sell", "hold", "close"]:
        logger.warning(
            f"[BINANCE] Invalid operation '{operation}' from AI for {account.name}"
        )
        save_ai_decision(
            db, account, decision, portfolio, executed=False, **decision_kwargs
        )
        return
    ...
    if operation == "hold":
        logger.info(
            f"[BINANCE] AI decided to HOLD for {account.name} - no action taken"
        )
        save_ai_decision(
            db, account, decision, portfolio, executed=True, **decision_kwargs
        )
        return
    ...
    try:
        if operation == "buy":
            ...
            if target_portion <= 0 or target_portion > 1:
                logger.warning(
                    f"[BINANCE] Invalid target_portion {target_portion} from AI for {account.name}"
                )
                save_ai_decision(
                    db, account, decision, portfolio, executed=False, **decision_kwargs
                )
            ...
            if result:
                ...
                save_ai_decision(
                    ...
                    portfolio,
                    executed=True,
                    hyperliquid_order_id=str(result.get("order_id"))
                ...
                try:
                    ...
                    try:
                        ...
                        trade_qty = (
                            Decimal(str(filled_qty)) if filled_qty > 0 else Decimal("0")
                        )
                        trade_price = (
                            Decimal(str(avg_price_val))
                            if avg_price_val > 0
                        ...
                        snapshot_db.commit()
                        logger.info(
                            f"[BINANCE] Close trade record saved for {account.name}"
                        )
                    finally:
                        snapshot_db.close()
                except Exception as trade_err:
                    logger.warning(
                        f"Failed to save Binance close trade record: {trade_err}"
                    )
            else:
        ...
        if order_result:
            ...
            save_ai_decision(
                ...
                portfolio,
                executed=executed,
                hyperliquid_order_id=str(order_result.get("order_id"))
                if order_result.get("order_id")
                else None,
                tp_order_id=str(order_result.get("tp_order_id"))
                if order_result.get("tp_order_id")
                else None,
                sl_order_id=str(order_result.get("sl_order_id"))
            ...
            if executed:
                ...
                logger.warning(
                    f"[BINANCE] {operation.upper()} order failed: {order_result}"
                )

    except Exception as e:
        logger.error(
            f"[BINANCE] Error executing {operation} for {symbol}: {e}", exc_info=True
        )
        save_ai_decision(
            db, account, decision, portfolio, executed=False, **decision_kwargs
        )
