...
def _get_client(wallet: BinanceWallet) -> BinanceTradingClient:
    ...
    if cache_key not in _client_cache:
        ...
        _client_cache[cache_key] = BinanceTradingClient(
            api_key=api_key, secret_key=secret_key, environment=wallet.environment
        )
...
async def setup_wallet(
    account_id: int, request: BinanceSetupRequest, db: Session = Depends(get_db)
):
    """
...
async def get_config(account_id: int, db: Session = Depends(get_db)):
    ...
    wallets = (
        db.query(BinanceWallet).filter(BinanceWallet.account_id == account_id).all()
    )
...
async def get_balance(
    account_id: int, environment: Optional[str] = None, db: Session = Depends(get_db)
):
    ...
    if not wallet:
        raise HTTPException(
            status_code=404, detail=f"No {environment} wallet configured"
        )
...
async def place_order(
    ...
    try:
        ...
        return {
            ...
            "tp_order": {"algo_id": result.get("tp_order_id")}
            if result.get("tp_order_id")
            else None,
            "sl_order": {"algo_id": result.get("sl_order_id")}
...
async def delete_wallet(
    account_id: int, environment: str, db: Session = Depends(get_db)
):
...
async def get_price(symbol: str):
    ...
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(
                status_code=response.status_code, detail="Failed to get price"
            )
...
async def get_all_binance_wallets(db: Session = Depends(get_db)):
    ...
    """
    wallets = db.query(BinanceWallet).filter(BinanceWallet.is_active == "true").all()

    result = []
    for wallet in wallets:
        account = (
            db.query(Account)
            .filter(Account.id == wallet.account_id, Account.is_deleted != True)
            .first()
        )
        if not account:
            continue
...
async def get_binance_trading_stats(
    ...
    try:
        ...
        logger.error(
            f"Failed to get Binance trading stats for account {account_id}: {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def check_rebate_eligibility(
    api_key: str, secret_key: str, environment: str = "mainnet"
):
    """
    ...
    try:
        client = BinanceTradingClient(
            api_key=api_key, secret_key=secret_key, environment=environment
        )
        ...
        return {
            ...
            "message": "Eligible for rebate"
            if result.get("eligible")
            else "Not eligible for rebate",
        }
...
async def update_symbol_watchlist(payload: BinanceSymbolSelectionRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to update Binance watchlist"
        )
